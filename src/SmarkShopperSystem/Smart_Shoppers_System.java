...
import User_Interfaces.StoreMangement_Form;
...
public class Smart_Shoppers_System extends javax.swing.JFrame {
    ...
    private void validLogin(String usr, String pswd, String path) throws FileNotFoundException, Exception {
        ...
        if(findUser(usr, pswd, path) && this.currentUser.getUserType().equals("Customer")){
            ...
            String originalStoreID = findStore(this.currentUser.getUsername());
    ...
    private String findStore(String userName){
        String storeNum = "";
        try {
            Maintain_Stores maintainStore = new Maintain_Stores();
            maintainStore.load(loginPath);
            
            for(Store store: maintainStore.stores){
                if(store.getManager().equals(userName)){
    ...
    private void initComponents() {
        ...
        jPanel2.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 73, 554, 240));
